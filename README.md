...
This is the Labview software of the acquisition control system (ACS) of the new astronomical instrument Simultaneous Polarimeter and Rapid Camera in Four Bands (SPARC4). This software is in version 1.2, and it is being developed to control two iXon Ultra EMCCD cameras of the Andor Technology company. It allows us to acquire image cubes for each camera, with the maximum cube size being 170 images full-frame (1024 x 1024 pixels). It is possible to concatenate image cubes with a minimal delay between them. Also, the writing of the data acquired by the cameras is done in parallel with the acquisition.
